...
{
    /**
     * Creates an Orders Checkout Pro order and redirects the buyer to Mercado Pago.
     *
     * @see OrderClient::create()
     */
    public function actionCheckoutPro(): Response
    {
        MercadoPagoConfig::setAccessToken(Yii::$app->params["mercadoPagoAccessToken"]);
        ...
        $request_options->setCustomHeaders([
            "X-Idempotency-Key: " . Yii::$app->security->generateRandomString()
        ]);

        $order = $client->create($request, $request_options);

        return $this->redirect($order->checkout_url);
    }
}